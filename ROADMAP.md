...
>
> Atualizado em 2026-09-01 (Turnstile ativado em produção com chaves reais da Cloudflare; corrigido também um `.vercelignore` desatualizado que fazia `vercel deploy` local subir `node_modules`/`.next` inteiros por ignorar o `.gitignore`. Depois: contador de rate limit dedicado — tabela `limites_taxa` substitui o `count()` sobre `LogAuditoria`. Depois: observabilidade no Django — recurso Rollbar próprio via Vercel Marketplace + `RollbarNotifierMiddleware`, fechando o último item pendente da varredura de 08-31 que não era um trade-off deliberado. Depois: integrações Neon desconectadas dos dois projetos (rollback pós-migração pro Supabase, confirmado estável) — achado no processo: a seção "Deploy (Vercel)" do `README.md` ainda dizia que o Django "continua na Neon", desatualizada desde a migração de 08-29; corrigida. Depois: **multi-tenant (organizações)** — `Organizacao`/`Membro`/`ConviteOrganizacao` novos, organização ativa vira claim no access token (`organizacaoId`/`papelOrganizacao`), troca de organização sem reautenticar, Django isolado por `organizacao_id`, RBAC de admin escopado por organização (não mais só o `papel` de sistema), convites por e-mail com aceite idempotente, seletor de organização e tela `/dashboard/organizacoes` no frontend — implementado em 5 fases, cada uma revisada e corrigida antes da próxima. Depois: achado no processo de deploy — os dois endpoints de autoatendimento LGPD (`/minha-conta/exportar` e `/minha-conta` DELETE) tinham ficado pra trás do multi-tenant: o export não incluía organizações/convites, e excluir a própria conta deixava a organização pessoal órfã (Membro cascade-deleta, Organizacao não) e não bloqueava quem era única pessoa dona de uma organização com outros membros; os dois corrigidos. **Implantado em produção em 2026-09-08** — o deploy tinha ficado 7 dias em `ERROR` sem ninguém notar: o `next build` morre em "Collecting page data" porque `src/lib/token.ts` faz `throw` na avaliação do módulo quando falta qualquer segredo, e a `JWT_CONVITE_ORGANIZACAO_SECRET` (nova, do multi-tenant) nunca foi criada no ambiente de produção — o CI do GitHub Actions passava verde porque lá a variável existe. Como o `npm run build` roda `prisma migrate deploy` ANTES do `next build`, os builds que falharam já tinham aplicado as migrations: produção ficou com schema novo e código velho. Destravado na ordem schema → dados → código nos dois serviços (backfill do Next.js: 3 organizações criadas; migrate + backfill do Django: 0 falhas). Depois: **Fase 00 do provedor (identidade por aplicação + JWKS)** — `Aplicacao` e `ChaveAssinatura` novos, e-mail único por aplicação em vez de global, `kid` no access token, `/.well-known/jwks.json`, Django validando por JWKS, rate limit e passkeys escopados por aplicação, rotação de chave sem deploy coordenado. Implementado numa branch (`fase-00-identidade-por-aplicacao`), **ainda não implantado em produção**. Depois, no mesmo dia: **incidente e correção de causa raiz** — o build de *preview* do PR da Fase 00 aplicou as migrations no banco de **produção** (Preview e Production dividem a mesma instância Supabase, e `prisma migrate deploy` rodava dentro do `build`, antes do `next build`); produção ficou com schema novo e código velho e o cadastro caiu com 500 até o merge. O `build` agora roda `scripts/checar-migrations.mjs`, que **verifica e nunca aplica**: em Production, migration pendente ou banco inacessível interrompe o build; em Preview e local, avisa e segue. Aplicar migration virou passo deliberado, igual ao lado Django. Contagens de teste: **236 no total** — 194 Vitest + 6 E2E Playwright + 36 Django.
...
> Implementado em 5 fases (schema/backfill → sessão/token → Django → RBAC → convites/UI), cada fase revisada e corrigida antes da seguinte. **Em produção desde 2026-09-08** — ver a nota do cabeçalho sobre os 7 dias de deploy quebrado.
...
- Backfill (`scripts/backfill-organizacoes.mjs` + `django/scripts/backfill_organizacoes.py`) cria a organização pessoal de cada conta pré-existente e reatribui `Projeto`/`Tarefa` antigos — idempotente, rodado em produção em 2026-09-08 (3 organizações criadas no lado Next.js, 0 falhas nos dois lados)
- Autoatendimento LGPD atualizado: o export (`POST /minha-conta/exportar`) agora inclui as organizações de que a pessoa é membro e os convites que ela criou; excluir a própria conta (`DELETE /minha-conta`) apaga junto a organização pessoal órfã (Membro cascade-deleta, Organizacao sozinha não) e bloqueia (409) quem é a única pessoa dona de uma organização com outros membros — mesma trava que já existia na remoção feita por um admin

### Identidade por aplicação (Fase 00 do provedor)
> Primeira fase de virar o sistema num provedor de auth vendável, em vez do auth de um app só. **Ainda não implantado em produção.**
- `Aplicacao` — o cliente que integra: `clientId` público (header `x-aplicacao-id`, seleciona o tenant mas nunca autoriza), segredo hasheado, lista de origens permitidas, RP ID de passkey, flag `padrao`
- `ChaveAssinatura` — conjunto de chaves RS256 com `kid`, privada cifrada em repouso (AES-256-GCM, mesma função usada no segredo TOTP); substitui a chave única em variável de ambiente
- **E-mail único por aplicação** (`@@unique([aplicacaoId, email])`) em vez de único no sistema — é o índice que separa "auth do meu app" de "auth como serviço"; slug de organização idem
- `GET /.well-known/jwks.json` — chaves públicas publicadas; o Django deixou de ter chave estática e passou a resolver por `kid` (`PyJWKClient` com cache), então rotacionar não exige mais deploy sincronizado dos dois serviços
- `aud` de volta no access token, agora como a aplicação — o claim removido em 01/09 por quebrar um consumidor, reintroduzido junto com o mecanismo que torna a mudança segura
- Rate limit por e-mail passou a levar a aplicação na chave: sem isso um cliente trancava o usuário do outro só martelando o mesmo endereço
- Passkeys por aplicação: RP ID e origens eram env vars globais, e uma credencial WebAuthn só vale na origem em que nasceu — com clientes em domínios próprios não dá pra ter um RP ID só
- Auditoria escopada por `aplicacaoId`: um admin vê a trilha da própria aplicação, nunca a dos outros clientes
- Requisição sem header cai na **aplicação padrão** — é o que manteve todo o front existente funcionando sem reescrever nenhuma tela
- `scripts/backfill-aplicacoes.mjs` (cria a aplicação padrão, atribui as linhas existentes, importa a chave do ambiente) e `scripts/rotacionar-chave-assinatura.mjs` (gerar → ativar → remover, na ordem que não derruba sessão)
- A migration destrutiva faz o próprio backfill em SQL, idempotente: o build da Vercel aplica todas as migrations pendentes de uma vez, então depender de um script externo ter rodado no meio quebraria o deploy — a lição de 08/09 aplicada como código

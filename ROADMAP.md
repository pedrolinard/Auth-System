...
> Gerado em 2026-07-13. Atualizado em 2026-07-14 (serviço de domínio Django/DRF com entidades reais — Projeto/Tarefa —, Postgres compartilhado, testes automatizados e telas de frontend). Cobre o que já foi implementado, o que falta fazer e melhorias sobre o que já existe.
...
### Dados
- Prisma + **Postgres local** (`Usuario`, `TokenAtualizacao`), com campos de MFA (`mfaAtivado`, `mfaSecret`) — migrado de SQLite, dados existentes preservados
...
- `django/` — serviço Django REST Framework sem login próprio: `comum/autenticacao.py` valida o JWT (RS256, `algorithms` fixo) e usa o claim `sub` como identidade
...
- `next.config.ts` encaminha `/api/dominio/*` para o Django via rewrite (mesma origem, sem CORS)
- **Postgres compartilhado** — mesma instância local do Next.js, database própria (`autenticacao_dominio`)
- **Testes automatizados** (`pytest-django`): `comum/tests/test_autenticacao.py` (token válido/expirado/adulterado/confusão de algoritmo) e `tarefas/tests/test_views.py` (CRUD + isolamento por usuário) — 13 testes
...
| 9 | Testes automatizados (unitários/integração) das rotas de auth do Next.js (o lado Django já tem, ver acima) | Alta |
